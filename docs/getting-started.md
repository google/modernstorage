...
ModernStorage has currently one artifact published to `mavenCentral()`:

* `com.google.modernstorage:mediastore`: The mediastore default artifact handles interactions with MediaStore. See [MediaStoreClient](mediaStoreClient.md) and [MediaActivityResults](mediaActivityResults.md)  for more details.
* More coming soon

## Java 8

ModernStorage requires [Java 8 bytecode](https://developer.android.com/studio/write/java8-support). To enable this feature add the following to your Gradle build script:
